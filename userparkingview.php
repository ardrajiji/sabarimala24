...
?>
    ...
    <div class="container py-lg-5 mt-sm-5 mt-3">
      ...
      <div class="row py-md-5 py-sm-3">
		...
		$uname=$_SESSION['username'];
			$qry="select username,phoneno from signup where username='$uname'"; 
  ...
  <label>PhoneNo:</label>
   <input type="text" name="phno" value="<?php if(!empty($res['phoneno'])){ echo $res['phoneno'];} ?>" />
  <br/>
 ...
 if(isset($_REQUEST['sub']))
 {
 	...
 	$qry="select status from parkingbooking where phoneno='$phno'";
 	...
 	if($rs['status']==1)
 	{
	 $qry="select Numday,vtype from parkingbooking where phone ='$phno'";
 	$res=mysql_query($qry);
	$rs=mysql_fetch_array($res);
	$numday=$rs['Numday'];
	$vechi=$rs['vtype'];
	$qry1="select * from parking where vechi_type='$vechi'";
 	$res1=mysql_query($qry1);
	$rs1=mysql_fetch_array($res1);
	$amt=$rs1['amount'];
	echo"<script>alert('Approved');</script>";
	$total_amount=$amt * $numday;
