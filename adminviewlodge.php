...
?>
    ...
    <div class="container py-lg-5 mt-sm-5 mt-3">
      ...
      <?php
			$qry="select * from lodgebooking where status='not approved'";
			...
			echo "<td>".$rs1['nam']."</td>";
			echo"<td>".$rs1['phno']."</td>";
			echo "<td>".$rs1['date1']."</td>";
			echo "<td>".$rs1['lodgename']."</td>";
			echo"<td>".$rs1['numberofroom']."</td>";
			echo "<td>".$rs1['noofday']."</td>";
			echo "<td>".$rs1['rt']."</td>";
			echo "<td>".$rs1['type']."</td>";
			echo "<td><input type='checkbox' name='check[$i]'value='".$rs1['phno']."'/></td>";
			//echo'<a href='.'Darshanam_allot.php?id='.$rs1['bookid'].'>approved</a><br>';
			//echo "<a href='Darshanam_allot.php?bookid=$id&cname=$name&bdate=$bd&phno=$phno'>approved</a>";
			echo "</tr>";
			$i++;
			}
			echo"</table>";
				 ...
				 if(isset($_POST['check']))
				  {
					  foreach($_POST['check'] as $value)
					  {
						  ...
						  $sql="update lodgebooking set status='approve' where phno=$value";
						  mysql_query($sql);
