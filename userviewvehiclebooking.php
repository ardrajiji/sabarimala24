...
?>
    ...
    <div class="container py-lg-5 mt-sm-5 mt-3">
     ...
     <table align="center" border="1" width="800" height="200">
     	...
     	<th>Status</th>
     	<?php
		 $re=mysql_query("select * from vehiclebooking where name='$uname'");
		 while($res=mysql_fetch_array($re))
		 {
		 $na=$res['date'];
		 $vn=$res['vehiclenumber'];
		 $dn=$res['dname'];
		 $sl=$res['startlocation'];
		 $el=$res['endlocation'];
		 $nd=$res['noofdays'];
		 $st=$res['status'];
			 echo "<tr><td>$na</td><td>$vn</td><td>$dn</td><td>$sl</td><td>$el</td><td>$nd</td><td>$st</td></tr>";
		 }
