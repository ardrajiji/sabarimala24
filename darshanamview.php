...
?>
    ...
    <div class="container py-lg-5 mt-sm-5 mt-3">
		...
		<div class="w3ls-titles text-center mb-5">
				<h3 class="title">darshanam view</h3>
				<span class="btmspn">
        ...
        <div class="col-md-6">
                <form id="contact-form" name="myForm" class="form" action="#" onsubmit="return validateForm()" method="POST">
                    <div class="form-group">
                     ...
                     <?php
			$qry="select username,phone from signup where username='$uname'";
			$rs=mysql_query($qry);
		 $res=mysql_fetch_array($rs);
			?>         
			
  <label>PhoneNo:</label>
   <input type="text" name="phno" value="<?php if(!empty($res['phone'])){ echo $res['phone'];} ?>" />
 ...
 if(isset($_REQUEST['sub']))
 {
 	...
 	$qry="select status from darsanambooking where phno='$phno'";
 	...
 	if($rs['status']==1)
 	{
	 $qry="select class,noofpersons from darsanambooking where phno='$phno'";
 	$res=mysql_query($qry);
	$rs=mysql_fetch_array($res);
	$class=$rs['class'];
	$nopr=$rs['noofpersons'];
	$qry1="select * from darshanam where class='$class'";
 	$res1=mysql_query($qry1);
	$rs1=mysql_fetch_array($res1);
	$amt=$rs1['amount'];
	echo"<script>alert('Approved');</script>";
	$total_amount=$amt * $nopr;
